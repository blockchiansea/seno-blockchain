...
# Gets the version from the result of "seno version"
...
def get_seno_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["seno", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/seno-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/seno-blockchain-gui/packages/gui/package.json")
