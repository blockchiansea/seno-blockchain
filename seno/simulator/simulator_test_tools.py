...
from seno.cmds.init_funcs import create_all_ssl
from seno.consensus.coinbase import create_puzzlehash_for_pk
from seno.daemon.server import WebSocketServer, daemon_launch_lock_path
from seno.simulator.full_node_simulator import FullNodeSimulator
...
from seno.simulator.start_simulator import async_main as start_simulator_main
from seno.types.blockchain_format.sized_bytes import bytes32
from seno.util.bech32m import encode_puzzle_hash
from seno.util.config import create_default_seno_config, load_config, save_config
from seno.util.ints import uint32
from seno.util.keychain import Keychain
from seno.util.lock import Lockfile
from seno.wallet.derive_keys import master_sk_to_wallet_sk
...
def create_config(
    ...
    # create seno directories
    create_default_seno_config(seno_root)
    create_all_ssl(
        seno_root,
    ...
    # load config
    config = load_config(seno_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(fingerprint), "txse")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(seno_root, "config.yaml", config)
    return config


async def start_simulator(seno_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=seno_root)
    await service.start()
...
async def get_full_seno_simulator(
    automated_testing: bool = False, seno_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int], None]:
    """
    A seno root directory can be provided, otherwise a temporary one is created.
    This test can either be run in automated mode or not, which determines which mode block tools run in.
    This test is fully interdependent and can be used without the rest of the seno test suite.
    Please refer to the documentation for more information.
    """
    # Create and setup temporary seno directories.
    if seno_root is None:
        seno_root = Path(tempfile.TemporaryDirectory().name)
    ...
    crt_path = seno_root / config["daemon_ssl"]["private_crt"]
    key_path = seno_root / config["daemon_ssl"]["private_key"]
    ca_crt_path = seno_root / config["private_ssl_ca"]["crt"]
    ca_key_path = seno_root / config["private_ssl_ca"]["key"]
    with Lockfile.create(daemon_launch_lock_path(seno_root)):
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(seno_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(seno_root, automated_testing):
            yield simulator, seno_root, config, mnemonic, fingerprint
