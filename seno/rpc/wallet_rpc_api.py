...
from seno.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from seno.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from seno.types.blockchain_format.coin import Coin, coin_as_list
...
from seno.util.errors import KeychainIsLocked
from seno.util.ints import uint8, uint32, uint64, uint16
...
from seno.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
from seno.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from seno.wallet.nft_wallet.nft_wallet import NFTWallet
from seno.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from seno.wallet.notification_store import Notification
...
from seno.wallet.trading.offer import Offer
from seno.wallet.transaction_record import TransactionRecord
from seno.wallet.util.address_type import AddressType, is_valid_address
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xse_target_address")
        pool_target = config["pool"].get("xse_target_address")
    ...
    async def create_new_wallet(self, request: Dict) -> EndpointResult:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from seno.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from seno.util.bech32m import bech32_decode, convertbits
        from seno.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSSenoConnection] = self.service.get_full_node_peer()
        if peer is None:
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xse")
        ...
        key = None
        if asset_id is not None and asset_id != "xse":
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xse_coin_list = request.get("xse_coins", None)
        xse_coins = None
        if xse_coin_list:
            xse_coins = set([Coin.from_json_dict(xse_coin) for xse_coin in xse_coin_list])
        xse_change_target = request.get("xse_change_target", None)
        if xse_change_target is not None:
            if xse_change_target[:2] == "xse":
                xse_change_ph = decode_puzzle_hash(xse_change_target)
            else:
                xse_change_ph = bytes32(hexstr_to_bytes(xse_change_target))
        else:
            xse_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_xse(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
