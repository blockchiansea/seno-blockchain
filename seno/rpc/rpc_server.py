...
from seno.rpc.util import wrap_http_handler
from seno.server.outbound_message import NodeType
from seno.server.server import SenoServer, ssl_context_for_client, ssl_context_for_server
...
from seno.util.config import str2bool
from seno.util.ints import uint16
from seno.util.json_util import dict_to_json_str
from seno.util.network import WebServer
from seno.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> SenoServer:
        ...
        # a property so as to be read only which allows SenoServer to satisfy
        # Optional[SenoServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        seno.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSSenoConnection) -> None:
...
def default_get_connections(server: SenoServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
