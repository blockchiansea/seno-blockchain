...
from seno.cmds.init_funcs import check_keys, seno_init, seno_full_version_str
from seno.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from seno.daemon.keychain_server import KeychainServer, keychain_commands
from seno.daemon.windows_signal import kill
from seno.plotters.plotters import get_available_plotters
from seno.plotting.util import add_plot_directory
from seno.server.server import ssl_context_for_root, ssl_context_for_server
from seno.ssl.create_ssl import get_mozilla_ca_crt
from seno.util.beta_metrics import BetaMetricsLogger
from seno.util.seno_logging import initialize_service_logging
from seno.util.config import load_config
from seno.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
from seno.util.json_util import dict_to_json_str
from seno.util.keychain import (
    Keychain,
    passphrase_requirements,
    supports_os_passphrase_storage,
)
from seno.util.lock import Lockfile, LockfileError
from seno.util.network import WebServer
from seno.util.service_groups import validate_service
from seno.util.setproctitle import setproctitle
from seno.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Seno.")
    quit()


log = logging.getLogger(__name__)

service_plotter = "seno_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "seno": "seno",
        "seno_data_layer": "start_data_layer",
        "seno_data_layer_http": "start_data_layer_http",
        "seno_wallet": "start_wallet",
        "seno_full_node": "start_full_node",
        "seno_harvester": "start_harvester",
        "seno_farmer": "start_farmer",
        "seno_introducer": "start_introducer",
        "seno_timelord": "start_timelord",
        "seno_timelord_launcher": "timelord_launcher",
        "seno_full_node_simulator": "start_simulator",
        "seno_seeder": "start_seeder",
        "seno_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Seno will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["seno", "plotters", plotter]
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SENO_ROOT
    os.environ["SENO_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with SENO_ROOT: {os.environ['SENO_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in seno_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    seno_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("seno_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"seno-blockchain version: {seno_full_version_str()}")
