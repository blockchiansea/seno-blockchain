...
from seno.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from seno.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            seno_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        seno_tx = dataclasses.replace(seno_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(seno_tx)
    ...
    async def create_tandem_xse_tx(
        ...
        if fee > amount_to_claim:
            seno_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(seno_coins)[0].name()
            ...
            message = None
            for spend in seno_tx.spend_bundle.coin_spends:
            ...
            seno_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in seno_coins])
            seno_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_seno_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_seno_to_claim = payment_amount

        need_seno_transaction = (fee > 0 or regular_seno_to_claim > 0) and (fee - regular_seno_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_seno_transaction:
                    if fee > regular_seno_to_claim:
                        seno_tx, _ = await self.create_tandem_xse_tx(
                            fee,
                            uint64(regular_seno_to_claim),
                    ...
                    elif regular_seno_to_claim > fee:
                        seno_tx, _ = await self.create_tandem_xse_tx(
                            fee, uint64(regular_seno_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        seno_spend_bundle = SpendBundle([], G2Element())
        if seno_tx is not None and seno_tx.spend_bundle is not None:
            seno_spend_bundle = seno_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if seno_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=seno_tx.confirmed_at_height,
                    created_at_time=seno_tx.created_at_time,
                    to_puzzle_hash=seno_tx.to_puzzle_hash,
                    amount=seno_tx.amount,
                    fee_amount=seno_tx.fee_amount,
                    confirmed=seno_tx.confirmed,
                    sent=seno_tx.sent,
                    spend_bundle=None,
                    additions=seno_tx.additions,
                    removals=seno_tx.removals,
                    wallet_id=seno_tx.wallet_id,
                    sent_to=seno_tx.sent_to,
                    trade_id=seno_tx.trade_id,
                    type=seno_tx.type,
                    name=seno_tx.name,
                    memos=[],
                )
