...
from seno.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward
from seno.consensus.coinbase import farmer_parent_id, pool_parent_id
from seno.consensus.constants import ConsensusConstants
from seno.data_layer.data_layer_wallet import DataLayerWallet
from seno.data_layer.dl_wallet_store import DataLayerStore
from seno.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
from seno.pools.pool_wallet import PoolWallet
from seno.protocols import wallet_protocol
from seno.protocols.wallet_protocol import CoinState
from seno.server.outbound_message import NodeType
...
from seno.util.db_wrapper import DBWrapper2
from seno.util.errors import Err
from seno.util.ints import uint8, uint32, uint64, uint128
from seno.util.lru_cache import LRUCache
from seno.util.path import path_from_root
from seno.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from seno.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from seno.wallet.cat_wallet.cat_wallet import CATWallet
from seno.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
from seno.wallet.derivation_record import DerivationRecord
from seno.wallet.derive_keys import (
...
from seno.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, create_fullpuz, match_did_puzzle
from seno.wallet.key_val_store import KeyValStore
from seno.wallet.nft_wallet.nft_info import NFTWalletInfo
from seno.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from seno.wallet.nft_wallet.nft_wallet import NFTWallet
from seno.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from seno.wallet.notification_manager import NotificationManager
from seno.wallet.outer_puzzles import AssetType
from seno.wallet.puzzle_drivers import PuzzleInfo
from seno.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
from seno.wallet.settings.user_settings import UserSettings
from seno.wallet.trade_manager import TradeManager
from seno.wallet.transaction_record import TransactionRecord
from seno.wallet.util.address_type import AddressType
from seno.wallet.util.compute_hints import compute_coin_hints
from seno.wallet.util.transaction_type import TransactionType
from seno.wallet.util.wallet_sync_utils import last_change_height_cs, PeerRequestException
from seno.wallet.util.wallet_types import WalletType
from seno.wallet.wallet import Wallet
from seno.wallet.wallet_blockchain import WalletBlockchain
from seno.wallet.wallet_coin_record import WalletCoinRecord
from seno.wallet.wallet_coin_store import WalletCoinStore
from seno.wallet.wallet_info import WalletInfo
from seno.wallet.wallet_interested_store import WalletInterestedStore
from seno.wallet.wallet_nft_store import WalletNftStore
from seno.wallet.wallet_pool_store import WalletPoolStore
from seno.wallet.wallet_puzzle_store import WalletPuzzleStore
from seno.wallet.wallet_retry_store import WalletRetryStore
from seno.wallet.wallet_transaction_store import WalletTransactionStore
from seno.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSSenoConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xse_spam_amount = self.config.get("xse_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xse_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xse_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xse_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSSenoConnection) -> Optional[bytes32]:
        # Get minter DID
