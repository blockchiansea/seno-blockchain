...
from seno.protocols.shared_protocol import Capability
import logging
import random
...
from seno.types.blockchain_format.coin import hash_coin_ids, Coin
...
from seno.util.merkle_set import confirm_not_included_already_hashed, confirm_included_already_hashed, MerkleSet
from seno.wallet.util.peer_request_cache import PeerRequestCache
...
async def fetch_last_tx_from_peer(height: uint32, peer: WSSenoConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSSenoConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSSenoConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSSenoConnection, bool]],
