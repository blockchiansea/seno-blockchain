...
from seno.types.blockchain_format.program import Program, INFINITE_COST
...
from seno.util.bech32m import bech32_decode, bech32_encode, convertbits
from seno.util.ints import uint64
from seno.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    match_puzzle,
    solve_puzzle,
...
from seno.wallet.util.puzzle_compression import (
    compress_object_with_puzzles,
    decompress_object_with_puzzles,
    lowest_best_version,
)
from seno.wallet.uncurried_puzzle import UncurriedPuzzle, uncurry_puzzle
...
class Offer:
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "xse" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
