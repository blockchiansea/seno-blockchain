...
from seno.protocols.wallet_protocol import CoinState
from seno.server.ws_connection import WSSenoConnection
...
from seno.types.coin_spend import CoinSpend
from seno.types.spend_bundle import SpendBundle
from seno.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from seno.util.hash import std_hash
from seno.util.ints import uint8, uint16, uint32, uint64, uint128
from seno.wallet.derivation_record import DerivationRecord
from seno.wallet.did_wallet import did_wallet_puzzles
from seno.wallet.lineage_proof import LineageProof
from seno.wallet.nft_wallet import nft_puzzles
from seno.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from seno.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from seno.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from seno.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from seno.wallet.payment import Payment
from seno.wallet.puzzle_drivers import PuzzleInfo, Solver
from seno.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from seno.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from seno.wallet.transaction_record import TransactionRecord
from seno.wallet.uncurried_puzzle import uncurry_puzzle
from seno.wallet.util.compute_memos import compute_memos
from seno.wallet.util.debug_spend_bundle import disassemble
from seno.wallet.util.transaction_type import TransactionType
from seno.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
from seno.wallet.wallet import Wallet
from seno.wallet.wallet_coin_record import WalletCoinRecord
from seno.wallet.wallet_info import WalletInfo
from seno.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSSenoConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSSenoConnection) -> None:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("Seno Signed Message", message))
    ...
    async def create_tandem_xse_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        seno_coins = await self.standard_wallet.select_coins(fee)
        seno_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
        ...
        assert seno_tx.spend_bundle is not None
        return seno_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, seno_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if seno_tx is not None and seno_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, seno_tx.spend_bundle])
            seno_tx = dataclasses.replace(seno_tx, spend_bundle=None)
        ...
        if seno_tx is not None:
            tx_list.append(seno_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            seno_tx = await self.create_tandem_xse_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, seno_tx
    ...
    async def mint_from_did(
        ...
        xse_coins: Optional[Set[Coin]] = None,
        xse_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xse spend that funds the transactions and pays fees.
        ...
        :param xse_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xse_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xse spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xse coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xse_coins is None:
            xse_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xse_coins) > 0
        ...
        # Create the xse spend to fund the minting.
        spend_value = sum([coin.amount for coin in xse_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xse_spends = []
        if xse_change_ph is None:
            xse_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xse_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xse_change_ph, "amount": change, "memos": [xse_change_ph]})
        ]

        first = True
        for xse_coin in xse_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xse_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xse_coins]
                message_list.append(
                    Coin(xse_coin.name(), xse_primaries[0]["puzzlehash"], xse_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xse_coins) > 1:
                    xse_announcement: Optional[Set[bytes]] = {message}
                else:
                    xse_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xse_primaries,
                    fee=fee,
                    coin_announcements=xse_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xse_coin.name(), message).name()
            ...
            xse_spends.append(CoinSpend(xse_coin, puzzle, solution))
        xse_spend = await self.standard_wallet.sign_transaction(xse_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xse_spend, *eve_spends])
        return total_spend

    async def mint_from_xse(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        funding_coin = xse_coins.copy().pop()
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # Annnouncements for eve spend. These are asserted by the xse spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xse_coin in xse_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xse_primaries + primaries,
                    fee=fee,
                    coin_announcements=xse_announcement if len(xse_coins) > 1 else None,
...
if TYPE_CHECKING:
    from seno.wallet.wallet_protocol import WalletProtocol
