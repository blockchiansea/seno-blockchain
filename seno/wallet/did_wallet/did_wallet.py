...
from seno.util.ints import uint64, uint32, uint8, uint128
...
from seno.wallet.did_wallet.did_info import DIDInfo
...
from seno.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from seno.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSSenoConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSSenoConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            seno_tx = await self.create_tandem_xse_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if seno_tx is not None and seno_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(seno_tx)
