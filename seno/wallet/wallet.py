...
from seno.full_node.bundle_tools import simple_solution_generator
...
from seno.wallet.puzzles.puzzle_utils import (
...
from seno.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_seno(self, seno_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if seno_amount < 0:
            utxos = await self.select_coins(uint64(abs(seno_amount)), exclude)
        else:
        ...
        seno_amount = spend_value + seno_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(seno_amount), "memos": []}
                ]
    ...
    # WSSenoConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSenoConnection
