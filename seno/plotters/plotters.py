...
from seno.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from seno.plotters.chiapos import get_chiapos_install_info, plot_seno
from seno.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SENO_ROOT.
    seno_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SENO_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("seno plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, seno_plotter_options, "chiapos", "Create a plot with the default seno plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'seno plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        ...
        plot_seno(args, seno_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, seno_root_path, root_path)
    elif args.plotter.startswith("bladebit"):
        plot_bladebit(args, seno_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(seno_root_path)
