...
from seno.plot_sync.delta import Delta
from seno.plot_sync.receiver import Receiver
from seno.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
from seno.protocols import farmer_protocol, harvester_protocol
from seno.protocols.pool_protocol import (
...
from seno.server.server import ssl_context_for_root
...
from seno.util.bech32m import decode_puzzle_hash
from seno.util.byte_types import hexstr_to_bytes
from seno.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from seno.util.errors import KeychainProxyConnectionFailure
from seno.util.hash import std_hash
from seno.util.ints import uint8, uint16, uint32, uint64
...
from seno.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'seno keys generate' or open the UI."
        try:
        ...
        if "xse_target_address" not in self.config:
            self.config = config["farmer"]
        if "xse_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xse_target_address" not in self.config or "xse_target_address" not in self.pool_config:
            log.debug("xse_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["xse_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["xse_target_address"]
    ...
    def on_disconnect(self, connection: ws.WSSenoConnection):
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["xse_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["xse_target_address"] = pool_target_encoded
