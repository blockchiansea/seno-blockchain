...
from seno.data_layer.data_layer_errors import OfferIntegrityError
from seno.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from seno.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from seno.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from seno.wallet.sign_coin_spends import sign_coin_spends
from seno.wallet.trading.offer import NotarizedPayment, Offer
from seno.wallet.transaction_record import TransactionRecord
from seno.wallet.util.compute_memos import compute_memos
from seno.wallet.util.merkle_utils import _simplify_merkle_proof
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSSenoConnection) -> CoinState:
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSSenoConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            seno_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, seno_tx.spend_bundle])
                ),
                dataclasses.replace(seno_tx, spend_bundle=None),
            ]
