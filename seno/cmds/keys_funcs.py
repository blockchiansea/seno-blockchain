...
from seno.cmds.passphrase_funcs import obtain_current_passphrase
from seno.daemon.client import connect_to_daemon_and_validate
...
from seno.util.errors import KeychainException
from seno.util.file_keyring import MAX_LABEL_LENGTH
from seno.util.ints import uint32
from seno.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
from seno.util.keyring_wrapper import KeyringWrapper
...
def generate_and_print():
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run seno keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'seno keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from seno.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using Seno.\n"
    )
