...
from seno.cmds.cmds_util import get_any_service_client
from seno.cmds.units import units
from seno.consensus.block_record import BlockRecord
from seno.rpc.farmer_rpc_client import FarmerRpcClient
from seno.rpc.full_node_rpc_client import FullNodeRpcClient
from seno.rpc.wallet_rpc_client import WalletRpcClient
from seno.util.misc import format_bytes
from seno.util.misc import format_minutes
from seno.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total seno farmed: {amounts['farmed_amount'] / units['seno']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['seno']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['seno']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'seno start wallet' and 'seno wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'seno wallet show'")
    else:
        print("Note: log into your key using 'seno wallet show' to see rewards for each key")
