...
from seno.plotting.util import add_plot_directory, validate_plot_size
...
def show_plots(root_path: Path):
    from seno.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'seno plots add -d [dir]' and remove with"
        + " 'seno plots remove -d [dir]'"
        + " Scan and check plots with 'seno plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    from seno.util.seno_logging import initialize_logging

    root_path: Path = ctx.obj["root_path"]
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'seno init'")
...
def create_cmd(
    ...
    from seno.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from seno.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from seno.plotting.util import remove_plot_directory
