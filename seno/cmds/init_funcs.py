...
from seno.consensus.coinbase import create_puzzlehash_for_pk
from seno.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from seno.util.config import (
    create_default_seno_config,
    initial_config_file,
    load_config,
...
from seno.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'seno keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "xse_target_address" not in config["farmer"]
        stop_searching_for_pool = "xse_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xse_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xse_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xse_target_address") or all_targets[-2] == config["pool"].get(
                    "xse_target_address"
                ):
        ...
        if "xse_target_address" not in config["farmer"]:
            print(
                f"Setting the xse destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["xse_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["xse_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['xse_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "xse_target_address" not in config["pool"]:
            print(f"Setting the xse destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["xse_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["xse_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['xse_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the XCH destination addresses, edit the `xse_target_address` entries in"
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(seno_ca_crt_path, seno_ca_crt, seno_ca_key_path, seno_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return seno_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def seno_version_number() -> Tuple[str, str, str, str]:
...
def seno_full_version_str() -> str:
    major, minor, patch, dev = seno_version_number()
...
def seno_init(
    ...
    *,
    ...
    """
    seno_root = os.environ.get("SENO_ROOT", None)
    if seno_root is not None:
        print(f"SENO_ROOT is set to {seno_root}")

    print(f"Seno directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SENO_ROOT is set, or if user has run seno init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'seno keys show --show-mnemonic-seed'")
