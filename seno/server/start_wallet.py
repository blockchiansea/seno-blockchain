...
from seno.rpc.wallet_rpc_api import WalletRpcApi
from seno.server.outbound_message import NodeType
from seno.server.start_service import RpcInfo, Service, async_run
from seno.types.peer_info import PeerInfo
from seno.util.seno_logging import initialize_service_logging
from seno.util.config import load_config_cli, load_config
...
from seno.wallet.wallet_node_api import WalletNodeAPI
...
async def async_main() -> int:
    ...
    if local_test is True:
        from seno.simulator.block_tools import test_constants
...
def main() -> int:
    freeze_support()
    if os.getenv("SENO_INSTRUMENT_WALLET", 0) != 0:
        from seno.util.task_timing import start_task_instrumentation, stop_task_instrumentation
        import atexit
