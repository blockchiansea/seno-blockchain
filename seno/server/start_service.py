...
from seno.daemon.server import service_launch_lock_path
from seno.util.lock import Lockfile, LockfileError
from seno.server.ssl_context import seno_ssl_ca_paths, private_ssl_ca_paths
...
from seno.cmds.init_funcs import seno_full_version_str
from seno.rpc.rpc_server import RpcApiProtocol, RpcServiceProtocol, start_rpc_server, RpcServer
from seno.server.outbound_message import NodeType
from seno.server.server import SenoServer
from seno.server.upnp import UPnP
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        self._log.info(f"seno-blockchain version: {seno_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        seno_ca_crt, seno_ca_key = seno_ssl_ca_paths(root_path, self.config)
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"seno_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for SenoServer to be closed")
