...
def solve_name_collision_problem(analysis):
    """
    There is a collision between the `seno` file name (which is the executable)
    and the `seno` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `seno` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("seno-blockchain"))[0]
...
# TODO: collapse all these entry points into one `seno_exec` entrypoint that accepts the server as a parameter

entry_points = ["seno.cmds.seno"] + [f"seno.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "seno.util.bip39"])
...
datas.append((f"{ROOT}/seno/util/english.txt", "seno/util"))
datas.append((f"{ROOT}/seno/util/initial-config.yaml", "seno/util"))
datas.append((f"{ROOT}/seno/wallet/puzzles/*.hex", "seno/wallet/puzzles"))
datas.append((f"{ROOT}/seno/ssl/*", "seno/ssl"))
...
add_binary("seno", f"{ROOT}/seno/cmds/seno.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/seno/daemon/server.py", COLLECT_ARGS)
