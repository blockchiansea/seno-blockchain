...
git submodule update --init seno-blockchain-gui

Set-Location -Path ".\seno-blockchain-gui" -PassThru
...
# Other than `seno-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @seno packages to make cache slim"
...
# Remove `packages/gui/node_modules/@seno` because it causes an error on later `electron-packager` command
Remove-Item "@seno" -Recurse -Force
