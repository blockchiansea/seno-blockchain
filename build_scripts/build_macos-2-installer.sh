...
LAST_EXIT_CODE=$?
ls -l dist/mac*/seno.app/Contents/Resources/app.asar
...
DMG_NAME="seno-${SENO_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/${DMG_NAME} dist/seno-${SENO_INSTALLER_VERSION}-arm64.dmg
  DMG_NAME=seno-${SENO_INSTALLER_VERSION}-arm64.dmg
fi
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id net.seno.blockchain \
...
# xcrun altool --notarize-app -f Seno-0.1.X.dmg --primary-bundle-id net.seno.blockchain -u username -p password
...
# xcrun stapler staple Seno-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Seno-0.1.X.dmg
