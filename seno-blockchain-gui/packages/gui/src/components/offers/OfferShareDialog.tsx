...
import { OfferTradeRecord } from '@seno/api';
import {
  Button,
  Checkbox,
  Dialog,
...
const log = debug('seno-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.seno.top';
...
async function postToOfferBin(
  ...
  if (testnet) {
    return 'https://www.seno.top/offers';
  }
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. XSE/RandoCoin
...
async function postToSpacescan(
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txse' : 'xse'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'txse' : 'xse'}/offer/${id}`;
}
...
const KeybaseTeamName = 'seno_offers';
