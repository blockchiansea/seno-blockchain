...
import { type OfferSummaryRecord } from '@seno/api';
import {
  Flex,
  FormatLargeNumber,
  StateColor,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xse', 'txse'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToSeno(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xse', 'txse'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToSeno(takerEntries[0][1])
