...
import { OfferSummaryRecord, OfferTradeRecord } from '@seno/api';
import {
  Box,
  Checkbox,
  Chip,
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: xseFee } = methods.getValues();

    const fee = cancelWithTransaction ? senoToMojo(xseFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TXSE';
                  ...
                  <Flex flexDirection="column" gap={4} height="100%">
                    <Flex flexDirection="column" gap={2} flexGrow={1}>
                      ...
                      <Typography variant="body1" color="textSecondary">
                        <Trans>
                          Create a file that you can use to trade XSE, Seno
