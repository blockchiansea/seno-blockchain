...
import type { CalculateRoyaltiesRequest, NFTInfo } from '@seno/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@seno/api-react';
import { catToMojo, senoToMojo } from '@seno/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXSE = useWatch({
    name: 'requested.xse',
  });

  const offeredXSE = useWatch({
    name: 'offered.xse',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXSE ?? [])
      ...
      .map(({ amount }) =>
        fungibleAssetFromAssetIdAndAmount('xse', senoToMojo(amount)),
      ),
    ...(requestedTokens ?? [])
