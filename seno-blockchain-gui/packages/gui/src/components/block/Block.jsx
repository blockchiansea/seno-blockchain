...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@seno/api-react';
...
export default function Block() {
  ...
  const poolReward = mojoToSeno(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToSeno(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const senoFees =
    blockRecord.fees !== undefined ? mojoToSeno(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: senoFees ? `${senoFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Seno blockchain
            </Trans>
          </Back>
        }
        action={
