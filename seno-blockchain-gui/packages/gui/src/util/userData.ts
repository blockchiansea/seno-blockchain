...
export function getUserDataDir(): string {
  const senoRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(senoRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const senoRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${senoRootUserDataPath}`);
  app.setPath('userData', senoRootUserDataPath);
}
