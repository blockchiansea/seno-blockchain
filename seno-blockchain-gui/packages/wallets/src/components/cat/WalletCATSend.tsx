...
import { useSpendCATMutation, useFarmBlockMutation } from '@seno/api-react';
import { SyncingStatus, toBech32m } from '@seno/api';
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(
        t`Cannot send seno to coloured address. Please enter a seno address.`
      );
    }

    if (address.includes('seno_addr') || address.includes('colour_desc')) {
      throw new Error(
    ...
    if (address.slice(0, 12) === 'seno_addr://') {
