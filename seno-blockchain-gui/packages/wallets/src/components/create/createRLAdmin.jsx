...
import { AlertDialog, senoToMojo } from '@seno/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      senoper_input.value === '' ||
      Number(senoper_input.value) === 0 ||
      !Number(senoper_input.value) ||
      isNaN(Number(senoper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const senoper = senoToMojo(senoper_input.value);
    const senoper_value = Number.parseInt(Number(senoper));
    ...
    // var fee = senoToMojo(fee_input.value);
