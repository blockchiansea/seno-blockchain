...
import type { Row } from '@seno/core';
import { WalletType, TransactionType, toBech32m } from '@seno/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToSeno(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
