...
import { TransactionType, WalletType } from '@seno/api';
import type { Transaction } from '@seno/api';
import { mojoToSeno, mojoToCAT, blockHeightToTimestamp } from '@seno/core';
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip:
        walletType === WalletType.CAT
          ? mojoToCAT(start)
          : mojoToSeno(start).toString(), // bignumber is not supported by react
    });
  });
