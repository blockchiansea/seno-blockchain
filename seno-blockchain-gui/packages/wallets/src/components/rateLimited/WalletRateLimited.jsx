...
import { AlertDialog, Card, Flex, senoToMojo, mojoToSenoLocaleString } from '@seno/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const senoper_input = ip_parsed.limit;
    ...
    const senoper_value = Number.parseInt(Number(senoper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (seno per interval):{' '}
                  {mojoToSenoLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToSenoLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = senoToMojo(amount_input.value);
    const fee = senoToMojo(fee_input.value);
